...
public class LegendaryCardMaker {
			...
			while ((line = br.readLine()) != null) {
			   if (line != null && !line.startsWith("#") && !line.isEmpty())
			   {
				   ...
				   if (!ignoreGenerate && line.startsWith("HCGENERATE;"))
				   {
					   ...
					   try
					   {
						   bwErr.write(Messages.getString("Messages.Generating")+": " + hc.getCardName(exportFolder) + "\n");
				   ...
				   if (!ignoreGenerate && line.startsWith("VCGENERATE;"))
				   {
					   ...
					   try
					   {
						   bwErr.write(Messages.getString("Messages.Generating")+": " + vc.getCardName(exportFolder) + "\n");
				   ...
				   if (!ignoreGenerate && line.startsWith("SCGENERATE;"))
				   {
					   try
					   {
						   bwErr.write(Messages.getString("Messages.Generating")+": " + sc.getCardName(exportFolder) + "\n");
	...
	public void outputVillainCard(BufferedWriter br, VillainCard vc) throws Exception
	{
		...
		if (vc.attack != null || vc.victory != null)
		{
			if (vc.attack != null)
			{
				br.write(Messages.getString("Skill.Attack")+" " + vc.attack);
				if (vc.victory != null)
				{
					br.write(", ");
				}
			...
			if (vc.victory != null)
			{
				br.write(Messages.getString("Skill.VictoryPoints")+" " + vc.victory);
			}
	...
	{
		System.out.println(Messages.getString("Logs.LoadingCustomTemplates"));
			...
			for (File f : dir.listFiles())
			{
				if (f.isDirectory())
				{
					...
					if (templateFile != null)
					{
						...
						System.out.println(Messages.getString("Logs.Loaded")+": "+ template.templateName);
					}
				}
			}
	...
	{
		System.out.println(Messages.getString("Logs.LoadingCustomStructures"));
		...
		if (dir.exists())
		{
			for (File f : dir.listFiles())
			{
				if (f.getName().endsWith(".xml"))
				{
					...
					System.out.println(Messages.getString("Logs.Loaded")+": " + template.name);
				}
			}
		}
	}
